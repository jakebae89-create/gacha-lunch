# 가챠밥 — 플레이 스토어 출시 가이드 (무료 버전)

이 앱은 **PWA(웹앱)**입니다. 구글 플레이에는 PWA를 안드로이드 앱으로 감싸는
**TWA(Trusted Web Activity)** 방식으로 올립니다. 아래 순서대로 진행하면 됩니다.

> 결정된 값
> - **패키지명(앱 ID)**: `app.gachabap.twa`
> - **배포 주소**: `https://jakebae89-create.github.io/gacha-lunch/`
...
| 매니페스트 | ✅ `manifest.webmanifest` | 가챠밥으로 갱신 완료 |
| 개인정보처리방침 페이지 | ✅ `privacy.html` | 연락처 이메일 입력 완료 |
| Digital Asset Links | ⏳ `.well-known/assetlinks.json` | 패키지명 입력 완료 / **서명키 지문만 입력 필요(3단계)** |
...
- 이 저장소(`gacha-lunch`)를 GitHub Pages로 배포 → `https://jakebae89-create.github.io/gacha-lunch/`
- 배포 후 아래가 정상 동작하는지 확인:
  - `https://jakebae89-create.github.io/gacha-lunch/index.html` (앱)
  - `https://jakebae89-create.github.io/gacha-lunch/manifest.webmanifest`
  - `https://jakebae89-create.github.io/gacha-lunch/privacy.html` (개인정보처리방침)
  - 서비스워커 등록 + 설치 배너 노출(설치형 PWA)

> ⚠️ **assetlinks.json 위치 주의**
> TWA 도메인 검증 파일은 반드시 **도메인 루트**에서 서비스돼야 합니다.
> 즉 `https://jakebae89-create.github.io/.well-known/assetlinks.json` (프로젝트 하위 경로 `/gacha-lunch/` 가 아님).
> - GitHub Pages **프로젝트 페이지**(`/gacha-lunch/`)만 쓰면 루트에 파일을 둘 수 없습니다.
> - 해결책 중 하나:
>   1. 사용자 페이지 저장소(`jakebae89-create.github.io`)의 루트 `/.well-known/assetlinks.json`에 배치, 또는
...
1. <https://www.pwabuilder.com> 접속 → 배포한 PWA 주소(`https://jakebae89-create.github.io/gacha-lunch/`) 입력 → **Start**
2. 매니페스트/서비스워커/보안 점검 결과 확인 (빨간 항목 있으면 보완)
3. **Package For Stores → Android(Google Play)** 선택
4. 패키지 옵션:
   - **Package ID(앱 패키지명)**: `app.gachabap.twa` (한번 정하면 변경 불가, 고유해야 함)
   - **App name**: 가챠밥
...
> bubblewrap init --manifest https://jakebae89-create.github.io/gacha-lunch/manifest.webmanifest
> bubblewrap build
> ```
...
2. 이 저장소의 `.well-known/assetlinks.json`에서 아래 값을 실제 값으로 교체
   - `package_name` → `app.gachabap.twa` (입력 완료)
   - `sha256_cert_fingerprints` → 서명키 SHA-256 지문 (**`REPLACE_WITH_YOUR_SIGNING_KEY_SHA256_FINGERPRINT` 자리에 입력 필요**)
3. 그 파일을 **도메인 루트** `https://jakebae89-create.github.io/.well-known/assetlinks.json`로 배포 (1단계 주의 참고)
...
- **피처 그래픽**: 1024×500 PNG/JPG (직접 제작 — 캡슐 가챠 머신 + 앱 이름 권장)
- **스크린샷**: 폰 2~8장 (메인 가챠 머신, 캡슐 채움, 캡슐 배출/당첨 모달, 퀴즈 화면 등)
  - 권장 비율 9:16, 최소 320px. 모바일 프리뷰로 캡처한 화면 활용 가능
- **카테고리**: 음식/식당 또는 라이프스타일
- **콘텐츠 등급(설문)**: 전체 이용가 예상
- **개인정보처리방침 URL**: `https://jakebae89-create.github.io/gacha-lunch/privacy.html`
