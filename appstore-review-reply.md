...
## 2026-08-15 18시대 — 재제출 완료, 심사 대기 중

아래 절차는 다 끝났습니다. 남은 건 애플의 답을 기다리는 것뿐입니다.

- 17:54 **앱 심사에 회신** 전송 — 영문 본문 + 첨부 3개(launch mp4 / final mp4 / 권한 png).
  붙여넣기용 txt를 BOM 포함으로 저장하는 바람에 첫 글자가 먹혀 "hank you for the review."로
  나갔지만 그대로 뒀습니다. 오타 하나 때문에 회신을 또 보내면 스레드만 지저분해집니다.
- **앱 심사 정보 메모**를 첫 제출 때 쓰던 옛 문구에서 7항목 본문으로 교체(남은 글자 19).
  메모 첨부는 **한 개만** 되므로 두 영상과 스크린샷을 이어붙인 `gacha-lunch-review-all.mp4`
  (73초, 7.1MB)를 올렸습니다.
- **빌드를 2 → 3으로 교체.** 붙어 있던 게 빌드 2였습니다 — 그대로 냈으면 중간지점 버그가
  살아 있는 빌드가 심사에 올라가고 영상과 앱이 어긋날 뻔했습니다.
- `앱 심사에 다시 제출` → 상태 **심사 대기 중**, 항목 iOS 앱 1.0 (3).

EU 거래자(DSA)는 제출을 막지 않았습니다. 다만 미신고 상태면 출시 후 EU에서 내려가므로
승인 전에 처리하거나 EU를 배포 대상에서 빼야 합니다.

## (기록) 제출 전까지의 경과 (2026-08-15)
